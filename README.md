...
- **56 automated checks in CI** ([`tests/audit.py`](tests/audit.py)) — structure and broken anchors, every referenced image existing and carrying alt text, JSON-LD validity, phone-number consistency, form integrity (hidden field and honeypot), hreflang reciprocity, repo-wide secret scanning, and a regression guard on the sanitiser. Dependency-free, so CI needs no install step.
...
tests/audit.py         56 checks, no dependencies
...
python3 tests/audit.py                # run the 56 checks
